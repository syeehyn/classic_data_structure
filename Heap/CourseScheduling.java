...
import java.util.*;
/**
 * the class contians the CourseScheduling object.
...
public class CourseScheduling {
    ...
    public static Student getStudent(String pid) {
        ...
        while (iter.hasNext()) {
            ...
            if (temp.getStudentID().equals(pid)) {
                return temp;
            }
        }
        return null;
        ...
        try {
            ...
            while (scLine.hasNextLine()) { //if the file has next line
                ...
                if (property.equals("register")) {
                    ...
                    String studentid = scWord.next();
                    String course = scWord.next();
                    int coin = Integer.parseInt(scWord.next());
                    Student toAdd = getStudent(studentid);
                    Registration adding = new Registration(toAdd, getCourse(course), coin);
                    // Containing condition.
                    if (toAdd.getmyWaitlist().contains(adding.getCourse())) {
                        printFail(toAdd, getCourse(course), false);
                    } else if (toAdd.getmyEnrolledCourses().contains(adding.getCourse())) {
                        printFail(toAdd, getCourse(course), true);
                    } else if (toAdd.getCoins() < coin) { // Coin condition.
                        printNoCoins(toAdd, adding.getCourse());
                    } else {
                        print(toAdd, getCourse(course), coin, false);
                        adding.getCourse().addToWaitlist(adding);
                    }
                    ...
                    int numStudent = scWord.nextInt();
                    for (int i = 0; i < numStudent; i++) {
                        for (Course courses : courseList) {
                            if (!courses.isFull()) {
                                Registration toProcess = courses.processWaitlist();
                                if (toProcess != null) {
                                    print(toProcess.getStudent(), toProcess.getCourse(),
                                            toProcess.getCoins(), true);
                                }
                            } else { // Capacity full condition.
                                printCapacity(courses);
                            }
                        }
                    }
